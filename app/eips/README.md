...
where `vm` is the package `"github.com/sacasnetwork/sacas/v20/x/evm/core/vm"`.
...
[`x/evm/`](https://github.com/sacasnetwork/sacas/tree/main/x/evm) folder
of **evmOS**.
...
sacasActivators = map[int]func(*vm.JumpTable){
...
```go
sacasEnabledEIPs = []int64{
    "sacas_0",
}
```

In this way, even though the custom activators defined $3$ new EIPs, we are going to activate only the number `sacas_0`
