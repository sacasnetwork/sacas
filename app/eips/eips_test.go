...
import (
	...
	"github.com/sacasnetwork/sacas/v20/app/eips/testdata"
	...
	integrationutils "github.com/sacasnetwork/sacas/v20/testutil/integration/sacas/utils"
...
func TestIPs(t *testing.T) {
	...
	RunSpecs(t, "SacasIPs Suite")
}

var _ = Describe("Improvement proposal sacas_0 - ", Ordered, func() {
	var (
		in network.Network
	...
	// Multiplier used to modify the opcodes associated with sacas_0 IP.
	...
	It("should enable the new IP", func() {
		...
		qRes, err = gh.GetEvmParams()
		Expect(err).To(BeNil(), "failed during query to evm params")
		Expect(qRes.Params.ExtraEIPs).To(ContainElement(newIP), "expected to have IP sacas_0 in evm params")
	})

	It("should change CREATE opcode constant gas after enabling sacas_0 IP", func() {
...
var _ = Describe("Improvement proposal sacas_1 - ", Ordered, func() {
	var (
		in network.Network
		tf factory.TxFactory
		gh grpc.Handler
	...
	// Multiplier used to modify the opcodes associated with sacas_1.
	...
	It("should enable the new IP", func() {
		...
		qRes, err = gh.GetEvmParams()
		Expect(err).To(BeNil(), "failed during query to evm params")
		Expect(qRes.Params.ExtraEIPs).To(ContainElement(newIP), "expected to have ip sacas_1 in evm params")
	})
...
var _ = Describe("Improvement proposal sacas_2 - ", Ordered, func() {
	var (
		in network.Network
		tf factory.TxFactory
		gh grpc.Handler
	...
	// Constant gas used to modify the opcodes associated with sacas_2.
	...
	It("should enable the new IP", func() {
		...
		qRes, err = gh.GetEvmParams()
		Expect(err).To(BeNil(), "failed during query to evm params")
		Expect(qRes.Params.ExtraEIPs).To(ContainElement(newIP), "expected to have ip sacas_2 in evm params")
	})
