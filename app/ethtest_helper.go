...
// EthSetup initializes a new SacasApp. A Nop logger is set in SacasApp.
func EthSetup(isCheckTx bool, patchGenesis func(*Sacas, simapp.GenesisState) simapp.GenesisState) *Sacas {
	return EthSetupWithDB(isCheckTx, patchGenesis, dbm.NewMemDB())
}

// EthSetupWithDB initializes a new SacasApp. A Nop logger is set in SacasApp.
func EthSetupWithDB(isCheckTx bool, patchGenesis func(*Sacas, simapp.GenesisState) simapp.GenesisState, db dbm.DB) *Sacas {
	chainID := utils.TestnetChainID + "-1"
	app := NewSacas(log.NewNopLogger(),
		db,
		nil,
		true,
		map[int64]bool{},
