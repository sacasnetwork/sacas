...
func PrepareEIP712CosmosTx(
	...
	accNumber := appSacas.AccountKeeper.GetAccount(ctx, from).GetAccountNumber()

	nonce, err := appSacas.AccountKeeper.GetSequence(ctx, from)
	if err != nil {
		return nil, err
	}
...
func createTypedData(args typedDataArgs, useLegacy bool) (apitypes.TypedData, error) {
	if useLegacy {
		...
		sacasCodec := codec.NewProtoCodec(registry)
