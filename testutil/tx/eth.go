...
func PrepareEthTx(
	...
	signer := ethtypes.LatestSignerForChainID(appSacas.EvmKeeper.ChainID())
	txFee := sdk.Coins{}
	txGasLimit := uint64(0)
...
func CreateEthTx(
	...
	chainID := appSacas.EvmKeeper.ChainID()

	// When we send multiple Ethereum Tx's in one Cosmos Tx, we need to increment the nonce for each one.
	nonce := appSacas.EvmKeeper.GetNonce(ctx, fromAddr) + uint64(nonceIncrement) //nolint:gosec // G115
	evmTxParams := &evmtypes.EvmTxArgs{
		...
		Amount:    amount,
		GasLimit:  100000,
		GasFeeCap: appSacas.FeeMarketKeeper.GetBaseFee(ctx),
