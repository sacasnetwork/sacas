...
var (
	...
	DefaultFee = sdk.NewCoin(utils.BaseDenom, sdkmath.NewIntFromUint64(uint64(feeAmt))) // 0.01 SACAS
)
...
type CosmosTxArgs struct {
	...
	// ChainID is the chain's id on cosmos format, e.g. 'sacas_1317-1'
	ChainID string
...
func signCosmosTx(
	...
	seq, err := appSacas.AccountKeeper.GetSequence(ctx, addr)
	if err != nil {
		return nil, err
	}
	...
	accNumber := appSacas.AccountKeeper.GetAccount(ctx, addr).GetAccountNumber()
