...
type ContractCallArgs struct {
	...
	Nonce *big.Int
	// Amount is the asacas amount to send in the transaction.
	Amount *big.Int
...
func DeployContract(
	...
	res, err := DeliverEthTx(sacasApp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := CheckEthTxResponse(res, sacasApp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func DeployContractWithFactory(
	...
	chainID := sacasApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	factoryNonce := sacasApp.EvmKeeper.GetNonce(ctx, factoryAddress)
	nonce := sacasApp.EvmKeeper.GetNonce(ctx, from)
...
func CallContract(ctx sdk.Context, sacasApp *app.Sacas, args ContractCallArgs) (res abci.ResponseDeliverTx, ethRes *evm.MsgEthereumTxResponse, err error) {
	var nonce uint64
	var (
		gasLimit = args.GasLimit
		cdc      = sacasApp.AppCodec()
	)
	...
	if args.Nonce == nil {
		nonce = sacasApp.EvmKeeper.GetNonce(ctx, addr)
	} else {
	...
	res, err = DeliverEthTx(sacasApp, args.PrivKey, msg)
	if err != nil {
