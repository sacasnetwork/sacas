...
// RegisterSacasERC20Coins uses the UnitNetwork to register the sacas token as an
...
func RegisterSacasERC20Coins(
	...
	sacasMetadata, found := network.App.BankKeeper.GetDenomMetaData(network.GetContext(), utils.BaseDenom)
	if !found {
		return erc20types.TokenPair{}, fmt.Errorf("expected sacas denom metadata")
	}

	_, err = network.App.Erc20Keeper.RegisterERC20Extension(network.GetContext(), sacasMetadata.Base)
	if err != nil {
		return erc20types.TokenPair{}, err
	}

	sacasDenomID := network.App.Erc20Keeper.GetDenomMap(network.GetContext(), bondDenom)
	tokenPair, ok := network.App.Erc20Keeper.GetTokenPair(network.GetContext(), sacasDenomID)
	if !ok {
		return erc20types.TokenPair{}, fmt.Errorf("expected sacas erc20 token pair")
	}
