...
import (
	...
	"github.com/sacasnetwork/sacas/v20/types"
	...
	commonnetwork "github.com/sacasnetwork/sacas/v20/testutil/integration/common/network"
...
func (n *IntegrationNetwork) configureAndInitChain() error {
	...
	// Create a new SacasApp with the following params
	sacasApp := createSacasApp(n.cfg.chainID, n.cfg.customBaseAppOpts...)
	...
	slashingParams, err := getValidatorsSlashingGen(validators, sacasApp.StakingKeeper)
	if err != nil {
		return err
	}
	...
	genesisState, err = customizeGenesis(sacasApp, n.cfg.customGenesisState, genesisState)
	if err != nil {
		return err
	}
	...
	header := cmtproto.Header{
		ChainID:            n.cfg.chainID,
		Height:             sacasApp.LastBlockHeight() + 1,
		AppHash:            sacasApp.LastCommitID().Hash,
		Time:               now,
	...
	if _, err := sacasApp.FinalizeBlock(req); err != nil {
		return err
	}

	// TODO - this might not be the best way to initilize the context
	n.ctx = sacasApp.BaseApp.NewContextLegacy(false, header)

	// Commit genesis changes
	if _, err := sacasApp.Commit(); err != nil {
		return err
	}
