...
import (
	...
	sacasibc "github.com/sacasnetwork/sacas/v20/ibc/testing"
	"github.com/sacasnetwork/sacas/v20/testutil/integration/common/network"
	ibcchain "github.com/sacasnetwork/sacas/v20/testutil/integration/ibc/chain"
)
...
type Coordinator interface {
	...
	GetPath(a, b string) *sacasibc.Path
...
// GetPath returns the transfer path for the chain ids 'a' and 'b'
func (c *IntegrationCoordinator) GetPath(a, b string) *sacasibc.Path {
	chainA := c.coord.GetChain(a)
	chainB := c.coord.GetChain(b)

	return sacasibc.NewTransferPath(chainA, chainB)
}
...
func (c *IntegrationCoordinator) Setup(a, b string) IBCConnection {
	path := c.GetPath(a, b)
	sacasibc.SetupPath(c.coord, path)
