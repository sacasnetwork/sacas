...
import (
	...
	"github.com/sacasnetwork/sacas/v20/crypto/hd"

	"github.com/sacasnetwork/sacas/v20/server/config"
...
type Config struct {
	...
	GenesisState      sacastypes.GenesisState // custom gensis state to provide
...
func DefaultConfig() Config {
	chainID := fmt.Sprintf("sacas_%d-1", cmtrand.Int63n(9999999999999)+1)
	dir, err := os.MkdirTemp("", "simapp")
	if err != nil {
		panic(fmt.Sprintf("failed creating temporary directory: %v", err))
	}
	defer os.RemoveAll(dir)
	app := app.NewSacas(log.NewNopLogger(), dbm.NewMemDB(), nil, true, nil, dir, 0, simutils.NewAppOptionsWithFlagHome(dir), baseapp.SetChainID(chainID))
	return Config{
		Codec:             app.AppCodec(),
		...
		NumValidators:     4,
		BondDenom:         "asacas",
		MinGasPrices:      fmt.Sprintf("0.000006%s", sacastypes.AttoSacas),
		AccountTokens:     sdk.TokensFromConsensusPower(1000000000000000000, sacastypes.PowerReduction),
		StakingTokens:     sdk.TokensFromConsensusPower(500000000000000000, sacastypes.PowerReduction),
		BondedTokens:      sdk.TokensFromConsensusPower(100000000000000000, sacastypes.PowerReduction),
...
// NewAppConstructor returns a new Sacas AppConstructor
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	if !sacastypes.IsValidChainID(cfg.ChainID) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "sacasd")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "sacascli")
		...
		genAccounts = append(genAccounts, &sacastypes.EthAccount{
		...
		customAppTemplate, _ := config.AppConfig(sacastypes.AttoSacas)
