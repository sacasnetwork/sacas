...
import (
	...
	"github.com/sacasnetwork/sacas/v20/precompiles/bank"
	"github.com/sacasnetwork/sacas/v20/precompiles/bank/testdata"
...
type IntegrationTestSuite struct {
	...
	sacasAddr, xmplAddr   common.Address
...
func (is *IntegrationTestSuite) SetupTest() {
	...
	is.sacasAddr = common.HexToAddress(tokenPair.Erc20Address)
...
var _ = Describe("Bank Extension -", func() {
	var (
		...
		sacasTotalSupply, _ = new(big.Int).SetString("200003000000000000000000", 10)
	...
	Context("Direct precompile queries", func() {
		...
		Context("totalSupply query", func() {
			It("should return the correct total supply", func() {
				...
				Expect(balances[0].Amount).To(Equal(sacasTotalSupply))
				Expect(balances[1].Amount).To(Equal(xmplTotalSupply))
			})
		})

		Context("supplyOf query", func() {
			It("should return the supply of Sacas", func() {
				queryArgs, supplyArgs := getTxAndCallArgs(directCall, contractData, bank.SupplyOfMethod, is.sacasAddr)
				_, ethRes, err := is.factory.CallContractAndCheckLogs(sender.Priv, queryArgs, supplyArgs, passCheck)
				Expect(err).ToNot(HaveOccurred(), "unexpected result calling contract")
				...
				Expect(out[0].(*big.Int)).To(Equal(sacasTotalSupply))
			})
	...
	Context("Calls from a contract", func() {
		...
		Context("supplyOf query", func() {
			It("should return the supply of Sacas", func() {
				queryArgs, supplyArgs := getTxAndCallArgs(contractCall, contractData, SupplyOfFunction, is.sacasAddr)
				_, ethRes, err := is.factory.CallContractAndCheckLogs(sender.Priv, queryArgs, supplyArgs, passCheck)
				Expect(err).ToNot(HaveOccurred(), "unexpected result calling contract")
