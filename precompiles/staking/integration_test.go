...
import (
	...
	compiledcontracts "github.com/sacasnetwork/sacas/v20/contracts"
	"github.com/sacasnetwork/sacas/v20/crypto/ethsecp256k1"
	"github.com/sacasnetwork/sacas/v20/precompiles/authorization"
	cmn "github.com/sacasnetwork/sacas/v20/precompiles/common"
	"github.com/sacasnetwork/sacas/v20/precompiles/distribution"
	"github.com/sacasnetwork/sacas/v20/precompiles/staking"
	"github.com/sacasnetwork/sacas/v20/precompiles/staking/testdata"
	"github.com/sacasnetwork/sacas/v20/precompiles/testutil"
	"github.com/sacasnetwork/sacas/v20/precompiles/testutil/contracts"
	sacasutil "github.com/sacasnetwork/sacas/v20/testutil"
	...
	testutils "github.com/sacasnetwork/sacas/v20/testutil/integration/sacas/utils"
	testutiltx "github.com/sacasnetwork/sacas/v20/testutil/tx"
...
var _ = Describe("Calling staking precompile directly", func() {
	...
	Describe("Execute approve transaction", func() {
		...
		It("should approve the undelegate method with 1 sacas", func() {
			granter := s.keyring.GetKey(0)
			grantee := s.keyring.GetKey(1)
		...
		It("should approve the redelegate method with 2 sacas", func() {
			granter := s.keyring.GetKey(0)
			grantee := s.keyring.GetKey(1)
		...
		It("should approve the cancel unbonding delegation method with 1 sacas", func() {
			granter := s.keyring.GetKey(0)
			grantee := s.keyring.GetKey(1)
	...
	Describe("Execute increase allowance transaction", func() {
		...
		It("Should increase the allowance of the delegate method with 1 sacas", func() {
			granter := s.keyring.GetKey(0)
			grantee := s.keyring.GetKey(1)
	...
	Describe("Execute decrease allowance transaction", func() {
		...
		It("Should decrease the allowance of the delegate method with 1 sacas", func() {
			granteeAddr := s.precompile.Address()
			granter := s.keyring.GetKey(0)
	...
	Describe("Calling precompile txs from a vesting account", func() {
		...
		BeforeEach(func() {
			...
			vestingAmtTotal := sacasutil.TestVestingSchedule.TotalVestingCoins
		...
		Context("before first vesting period - all tokens locked and unvested", func() {
			BeforeEach(func() {
				...
				Expect(unvested).To(Equal(sacasutil.TestVestingSchedule.TotalVestingCoins), "expected different unvested coins")
			...
			It("Should be able to delegate tokens not involved in vesting schedule", func() {
				...
				Expect(err).To(BeNil())
				Expect(*balRes.Balance).To(Equal(accountGasCoverage[0].Add(sacasutil.TestVestingSchedule.TotalVestingCoins[0]).Add(coinsToDelegate[0])))
				...
				Expect(err).To(BeNil())
				Expect(balRes.Balance.IsGTE(sacasutil.TestVestingSchedule.TotalVestingCoins[0])).To(BeTrue())
			})
		})
		...
		Context("after first vesting period and before lockup - some vested tokens, but still all locked", func() {
			BeforeEach(func() {
				...
				cliffDuration := time.Duration(sacasutil.TestVestingSchedule.CliffPeriodLength)
				Expect(s.network.NextBlockAfter(cliffDuration * time.Second)).To(BeNil())
				ctx := s.network.GetContext()

				// Check if some, but not all tokens are vested
				vested = clawbackAccount.GetVestedCoins(ctx.BlockTime())
				expVested := sdk.NewCoins(sdk.NewCoin(s.bondDenom, sacasutil.TestVestingSchedule.VestedCoinsPerPeriod[0].Amount.Mul(math.NewInt(sacasutil.TestVestingSchedule.CliffMonths))))
				Expect(vested).NotTo(Equal(sacasutil.TestVestingSchedule.TotalVestingCoins), "expected some tokens to have been vested")
			...
			It("Should be able to delegate locked vested tokens + free tokens (not in vesting schedule)", func() {
				...
				balRes, err := s.grpcHandler.GetBalance(vestAcc.Bytes(), s.bondDenom)
				Expect(err).To(BeNil())
				Expect(*balRes.Balance).To(Equal(accountGasCoverage[0].Add(sacasutil.TestVestingSchedule.TotalVestingCoins[0]).Add(amt[0])))
		...
		Context("Between first and second lockup periods - vested coins are unlocked", func() {
			BeforeEach(func() {
				...
				vestDuration := time.Duration(sacasutil.TestVestingSchedule.LockupPeriodLength)
				...
				expVested := sdk.NewCoins(sdk.NewCoin(s.bondDenom, sacasutil.TestVestingSchedule.VestedCoinsPerPeriod[0].Amount.Mul(math.NewInt(sacasutil.TestVestingSchedule.LockupMonths))))
				expUnlockedVested := expVested

				Expect(vested).NotTo(Equal(sacasutil.TestVestingSchedule.TotalVestingCoins), "expected not all tokens to be vested")
				...
				Expect(unlocked).To(Equal(sacasutil.TestVestingSchedule.UnlockedCoinsPerLockup))
	...
	Describe("Redelegations queries", func() {
		...
		It("should return an empty array if no redelegation is found for the given source validator", func() {
			...
			// possible (see https://github.com/sacasnetwork/cosmos-sdk/blob/e773cf768844c87245d0c737cda1893a2819dd89/x/staking/keeper/querier.go#L361-L373):
			//
	...
	Context("delegating", func() {
		...
		Describe("delegation from a vesting account", func() {
			...
			BeforeEach(func() {
				...
				// Check if all tokens are unvested at vestingStart
				totalVestingCoins := sacasutil.TestVestingSchedule.TotalVestingCoins
				unvested = clawbackAccount.GetVestingCoins(ctx.BlockTime())
				vested = clawbackAccount.GetVestedCoins(ctx.BlockTime())
			...
			Context("before first vesting period - all tokens locked and unvested", func() {
				BeforeEach(func() {
					...
					Expect(unvested).To(Equal(sacasutil.TestVestingSchedule.TotalVestingCoins))
			...
			Context("after first vesting period and before lockup - some vested tokens, but still all locked", func() {
				BeforeEach(func() {
					...
					expVested := sdk.NewCoins(sdk.NewCoin(s.bondDenom, sacasutil.TestVestingSchedule.VestedCoinsPerPeriod[0].Amount.Mul(math.NewInt(sacasutil.TestVestingSchedule.CliffMonths))))
					Expect(vested).NotTo(Equal(sacasutil.TestVestingSchedule.TotalVestingCoins))
