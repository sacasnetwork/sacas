...
import (
	...
	compiledcontracts "github.com/sacasnetwork/sacas/v15/contracts"
	"github.com/sacasnetwork/sacas/v15/precompiles/authorization"
	cmn "github.com/sacasnetwork/sacas/v15/precompiles/common"
	"github.com/sacasnetwork/sacas/v15/precompiles/distribution"
	"github.com/sacasnetwork/sacas/v15/precompiles/staking"
	"github.com/sacasnetwork/sacas/v15/precompiles/staking/testdata"
...
var _ = Describe("Calling staking precompile directly", func() {
	...
	Describe("to revoke an approval", func() {
		...
		It("should not revoke the approval if sent by someone else than the granter", func() {
			...
			err := sacasutil.FundAccountWithBaseDenom(s.ctx, s.app.BankKeeper, differentAddr.Bytes(), 1e18)
...
var _ = Describe("Calling staking precompile via Solidity", func() {
	...
	Context("delegating", func() {
		...
		Context("with approval set", func() {
			...
			It("should not delegate when sending from a different address", func() {
				...
				err := sacasutil.FundAccountWithBaseDenom(s.ctx, s.app.BankKeeper, newAddr, 1e18)
	...
	Context("querying delegation", func() {
		...
		It("which does not exist should return an empty delegation", func() {
			...
			Expect(delOut.Balance.Denom).To(Equal("asacas"), "expected a different delegation balance")
		})
		...
		It("which exists should return the delegation", func() {
			...
			Expect(delOut.Balance).To(Equal(
				cmn.Coin{Denom: "asacas", Amount: big.NewInt(1e18)}),
...
var _ = Describe("Batching cosmos and eth interactions", func() {
	...
	BeforeEach(func() {
		...
		contractAddr, err = sacasutil.DeployContract(s.ctx, s.app, s.privKey, s.queryClientEVM, testdata.StakingCallerContract)
		Expect(err).To(BeNil(), "error while deploying the StakingCaller contract")

		// Deploy ERC20 contract
		erc20ContractAddr, err = sacasutil.DeployContract(s.ctx, s.app, s.privKey, s.queryClientEVM, erc20Contract,
