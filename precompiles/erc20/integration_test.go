...
var _ = Describe("ERC20 Extension -", func() {
	...
	BeforeEach(func() {
		...
		Expect(common.HexToAddress(erc20Params.NativePrecompiles[0])).To(Equal(common.HexToAddress(erc20types.WSACContractTestnet)))
	...
	Context("basic functionality -", func() {
		...
		When("calling reverter contract", func() {
			Context("in a direct call to the WSAC contract", func() {
				var (
				...
				DescribeTable("it should revert token transfer from the WSAC contract", func(before bool, after bool) {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
					amountToSend := big.NewInt(100)
				...
				It("it should send token transfer and send from WSAC contract", func() {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
				...
				DescribeTable("it should revert token transfer and send from WSAC contract", func(before bool, after bool) {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
					balRes, err := is.handler.GetBalance(receiver.Bytes(), is.bondDenom)
					Expect(err).To(BeNil())
