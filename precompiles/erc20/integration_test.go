...
import (
	...
	auth "github.com/sacasnetwork/sacas/v20/precompiles/authorization"
	"github.com/sacasnetwork/sacas/v20/precompiles/erc20"
	"github.com/sacasnetwork/sacas/v20/precompiles/erc20/testdata"
	...
	utiltx "github.com/sacasnetwork/sacas/v20/testutil/tx"
...
var _ = Describe("ERC20 Extension -", func() {
	...
	BeforeEach(func() {
		...
		Expect(common.HexToAddress(erc20Params.NativePrecompiles[0])).To(Equal(common.HexToAddress(erc20types.WSACASContractTestnet)))

		wsacasAddress = common.HexToAddress(erc20Params.NativePrecompiles[0])
	...
	Context("basic functionality -", func() {
		...
		When("calling reverter contract", func() {
			Context("in a direct call to the WSACAS contract", func() {
				var (
				...
				DescribeTable("it should revert token transfer from the WSACAS contract", func(before bool, after bool) {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
					amountToSend := big.NewInt(100)
				...
				It("it should send token transfer and send from WSACAS contract", func() {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
				...
				DescribeTable("it should revert token transfer and send from WSACAS contract", func(before bool, after bool) {
					sender := is.keyring.GetKey(0)
					receiver := is.keyring.GetAddr(1)
					balRes, err := is.handler.GetBalance(receiver.Bytes(), is.bondDenom)
					Expect(err).To(BeNil())
		...
		When("querying allowance", func() {
			...
			When("querying the allowance for the own address", func() {
				...
				//
				// For reference see this comment: https://github.com/sacasnetwork/sacas/pull/2088#discussion_r1407646217
...
var _ = Describe("ERC20 Extension migration Flows -", func() {
	...
	When("using Sacas (not wSacas) in smart contracts", func() {
		It("should be using straight Sacas for sending funds in smart contracts", func() {
			Skip("will be addressed in follow-up PR")
