...
import (
	...
	"github.com/sacasnetwork/sacas/v20/precompiles/vesting"
...
var (
	...
	toAddr           = sacasutiltx.GenerateAddress()
	funderAddr       = sacasutiltx.GenerateAddress()
	diffFunderAddr   = sacasutiltx.GenerateAddress()
...
func (s *PrecompileTestSuite) TestCreateClawbackVestingAccount() {
	...
	testCases := []struct {
		...
		},
		{
			...
			malleate: func() []interface{} {
				differentAddr := sacasutiltx.GenerateAddress()
				return []interface{}{
					funderAddr,
					differentAddr,
					false,
	...
	testCases := []struct {
		...
		},
		{
			...
			func() []interface{} {
				...
				err = sacasutil.FundAccount(ctx, s.network.App.BankKeeper, toAddr.Bytes(), sdk.NewCoins(sdk.NewCoin(utils.BaseDenom, math.NewInt(100))))
				return []interface{}{
					s.keyring.GetAddr(0),
					toAddr,
					uint64(time.Now().Unix()), //nolint:gosec // G115
	...
	testCases := []struct {
		...
		},
		{
			...
			func(data []byte) {
				...
				_, ok := account.(sacastypes.EthAccountI)
				s.Require().True(ok)
			},
			false,
			"",
