...
import (
	...
	sacascontracts "github.com/sacasnetwork/sacas/v15/contracts"
	sacasibc "github.com/sacasnetwork/sacas/v15/ibc/testing"
	"github.com/sacasnetwork/sacas/v15/precompiles/authorization"
	cmn "github.com/sacasnetwork/sacas/v15/precompiles/common"
	"github.com/sacasnetwork/sacas/v15/precompiles/ics20"
	"github.com/sacasnetwork/sacas/v15/precompiles/testutil"
	"github.com/sacasnetwork/sacas/v15/precompiles/testutil/contracts"
	sacasutil "github.com/sacasnetwork/sacas/v15/testutil"
	sacasutiltx "github.com/sacasnetwork/sacas/v15/testutil/tx"
...
func (s *PrecompileTestSuite) NewTestChainWithValSet(coord *ibctesting.Coordinator, valSet *tmtypes.ValidatorSet, signers map[string]tmtypes.PrivValidator) *ibctesting.TestChain {
	...
	addr, priv := sacasutiltx.NewAddrKey()
	s.privKey = priv
	s.address = addr
	// differentAddr is an address generated for testing purposes that e.g. raises the different origin error
	s.differentAddr = sacasutiltx.GenerateAddress()
	s.signer = sacasutiltx.NewSigner(priv)
...
func (s *PrecompileTestSuite) NewTransferAuthorizationWithAllocations(ctx sdk.Context, app *sacasapp.Sacas, grantee, granter common.Address, allocations []transfertypes.Allocation) error {
...
func (s *PrecompileTestSuite) NewTransferAuthorization(ctx sdk.Context, app *sacasapp.Sacas, grantee, granter common.Address, path *ibctesting.Path, coins sdk.Coins, allowList []string) error {
...
func (s *PrecompileTestSuite) setupIBCTest() {
	...
	s.app = s.chainA.App.(*sacasapp.Sacas)
	...
	s.transferPath = sacasibc.NewTransferPath(s.chainA, s.chainB) // clientID, connectionID, channelID empty
	sacasibc.SetupPath(s.coordinator, s.transferPath)             // clientID, connectionID, channelID filled
...
// TODO upstream this change to sacas (adding gasPrice)
// DeployContract deploys a contract with the provided private key,
// compiled contract data and constructor arguments
func DeployContract(
	ctx sdk.Context,
	sacasApp *sacasapp.Sacas,
	...
	chainID := sacasApp.EvmKeeper.ChainID()
	from := common.BytesToAddress(priv.PubKey().Address().Bytes())
	nonce := sacasApp.EvmKeeper.GetNonce(ctx, from)
	...
	gas, err := sacasutiltx.GasLimit(ctx, from, data, queryClientEvm)
	if err != nil {
		return common.Address{}, err
	}
	...
	msgEthereumTx := evmtypes.NewTx(&evmtypes.EvmTxArgs{
		...
		Nonce:     nonce,
		GasLimit:  gas,
		GasFeeCap: sacasApp.FeeMarketKeeper.GetBaseFee(ctx),
	...
	res, err := sacasutil.DeliverEthTx(sacasApp, priv, msgEthereumTx)
	if err != nil {
		return common.Address{}, err
	}

	if _, err := sacasutil.CheckEthTxResponse(res, sacasApp.AppCodec()); err != nil {
		return common.Address{}, err
	}
...
func (s *PrecompileTestSuite) DeployERC20Contract(chain *ibctesting.TestChain, name, symbol string, decimals uint8) (common.Address, error) {
	addr, err := DeployContract(
		...
		gasPrice,
		s.queryClientEVM,
		sacascontracts.ERC20MinterBurnerDecimalsContract,
		name,
		symbol,
		decimals,
	)
...
func (s *PrecompileTestSuite) setupERC20ContractTests(amount *big.Int) common.Address {
	...
	defaultERC20CallArgs := contracts.CallArgs{
		...
		ContractABI:  sacascontracts.ERC20MinterBurnerDecimalsContract.ABI,
		PrivKey:      s.privKey,
		GasPrice:     gasPrice,
	}
	...
	mintCheck := testutil.LogCheckArgs{
		ABIEvents: sacascontracts.ERC20MinterBurnerDecimalsContract.ABI.Events,
	...
	balance := s.app.Erc20Keeper.BalanceOf(
		s.chainA.GetContext(),
		sacascontracts.ERC20MinterBurnerDecimalsContract.ABI,
		erc20Addr,
		s.address,
	)
