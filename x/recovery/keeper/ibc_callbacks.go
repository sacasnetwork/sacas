...
import (
	...
	"github.com/sacasnetwork/sacas/v15/ibc"
...
//   - sends over all Sacas native tokens
//
...
func (k Keeper) OnRecvPacket(
	...
	// Get addresses in `sacas1` and the original bech32 format
	...
	// a sender's own account on Sacas (sender == recipient)
	...
	k.bankKeeper.IterateAccountBalances(ctx, recipient, func(coin sdk.Coin) (stop bool) {
		...
		packetTransfer := &transfertypes.MsgTransfer{
			...
			Sender:           recipient.String(),        // recipient is the address in the Sacas chain
...
// the IBC denomination, i.e port and channel on Sacas for the voucher. It
// returns an error if:
