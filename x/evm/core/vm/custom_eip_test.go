	...
	testCases := []struct {
		...
		},
		{
			...
			map[string]func(*JumpTable){
				"sacas_0": func(jt *JumpTable) {},
			},
			true,
			"",
			func() {
				eips := GetActivatorsEipNames()
				require.ElementsMatch(t, append(eips_snapshot, "sacas_0"), eips, "expected eips number to be equal")
			},
		},
		{
			"success - multiple new activators",
			map[string]func(*JumpTable){
				"sacas_1": func(jt *JumpTable) {},
				"sacas_2": func(jt *JumpTable) {},
			},
			true,
			"",
			func() {
				...
				require.ElementsMatch(t, append(eips_snapshot, "sacas_0", "sacas_1", "sacas_2"), eips, "expected eips number to be equal")
			},
		},
		{
