...
func CalculateEpochMintProvision(
	...
	// Multiply epochMintProvision with power reduction (10^18 for sacas) as the
	// calculation is based on `sacas` and the issued tokens need to be given in
	// `asacas`
	epochProvision = epochProvision.Mul(math.LegacyNewDecFromInt(sacastypes.PowerReduction))
