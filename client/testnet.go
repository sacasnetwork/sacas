...
import (
	...
	"github.com/sacasnetwork/sacas/v20/testutil/network"
)
...
Example:
	sacasd testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
...
Example:
	sacasd testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	if args.chainID == "" {
		args.chainID = fmt.Sprintf("ethermint_%d-1", cmtrand.Int63n(9999999999999)+1)
	}
	...
	for i := 0; i < args.numValidators; i++ {
		...
		accStakingTokens := sdk.TokensFromConsensusPower(5000, sacastypes.PowerReduction)
		coins := sdk.Coins{
			sdk.NewCoin(sacastypes.AttoSacas, accStakingTokens),
		}
		...
		valTokens := sdk.TokensFromConsensusPower(100, sacastypes.PowerReduction)
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			addr.String(),
			valPubKeys[i],
			sdk.NewCoin(sacastypes.AttoSacas, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(sacastypes.AttoSacas)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, sacastypes.AttoSacas, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
