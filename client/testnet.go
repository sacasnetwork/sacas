...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		coins := sdk.Coins{
			sdk.NewCoin(evmostypes.AttoSacas, accStakingTokens),
		}
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			addr.String(),
			valPubKeys[i],
			sdk.NewCoin(evmostypes.AttoSacas, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(evmostypes.AttoSacas)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, evmostypes.AttoSacas, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
