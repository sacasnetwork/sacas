...
import (
	...
	sacas "github.com/sacasnetwork/sacas/v20/types"
...
func AddrCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf(
			`$ %s debug addr sacas1qqqqhe5pnaq5qq39wqkn957aydnrm45sdn8583
$ %s debug addr 0x00000Be6819f41400225702D32d3dd23663Dd690 --prefix sacas`, version.AppName, version.AppName),
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			addrString := args[0]
			switch {
	...
	cmd.Flags().String(flagPrefix, "", "Bech32 encoded account prefix, for example sacas, sacasvaloper")
	return cmd
}
...
func LegacyEIP712Cmd() *cobra.Command {
	return &cobra.Command{
		...
		Example: fmt.Sprintf(`$ %s debug legacy-eip712 tx.json --chain-id sacasd_1317-1`, version.AppName),
		Args:    cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			chainID, err := sacas.ParseChainID(clientCtx.ChainID)
			if err != nil {
