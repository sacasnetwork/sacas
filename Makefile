...
HTTPS_GIT := https://github.com/sacasnetwork/sacas.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=sacas \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(SACAS_BINARY) \
...
build-docker:
	...
	mkdir -p ./build/.sacasd
	echo '#!/usr/bin/env bash' > ./build/sacasd
	echo "IMAGE_NAME=${DOCKER_IMAGE}:${COMMIT_HASH}" >> ./build/sacasd
	echo 'SCRIPT_PATH=$$(cd $$(dirname $$0) && pwd -P)' >> ./build/sacasd
	echo 'docker run -it --rm -v $${SCRIPT_PATH}/.sacasd:/home/sacas/.sacasd $$IMAGE_NAME sacasd "$$@"' >> ./build/sacasd
	chmod +x ./build/sacasd
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/sacasnetwork/sacas"
	godoc -http=:6060
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(SACAS_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/sacas:Z sacas/node "./sacasd testnet init-files --v 4 -o /sacas --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\sacasd:/sacas\Z sacas/node "./sacasd tendermint unsafe-reset-all --home=/sacas"
	@docker run --rm -v $(CURDIR)\build\node1\sacasd:/sacas\Z sacas/node "./sacasd tendermint unsafe-reset-all --home=/sacas"
	@docker run --rm -v $(CURDIR)\build\node2\sacasd:/sacas\Z sacas/node "./sacasd tendermint unsafe-reset-all --home=/sacas"
	@docker run --rm -v $(CURDIR)\build\node3\sacasd:/sacas\Z sacas/node "./sacasd tendermint unsafe-reset-all --home=/sacas"
else
	@docker run --rm -v $(CURDIR)/build/node0/sacasd:/sacas:Z sacas/node "./sacasd tendermint unsafe-reset-all --home=/sacas"
	@docker run --rm -v $(CURDIR)/build/node1/sacasd:/sacas:Z sacas/node "./sacasd tendermint unsafe-reset-all --home=/sacas"
	@docker run --rm -v $(CURDIR)/build/node2/sacasd:/sacas:Z sacas/node "./sacasd tendermint unsafe-reset-all --home=/sacas"
	@docker run --rm -v $(CURDIR)/build/node3/sacasd:/sacas:Z sacas/node "./sacasd tendermint unsafe-reset-all --home=/sacas"
endif
...
PACKAGE_NAME:=github.com/sacasnetwork/sacas
