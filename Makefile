...
# Deps
DEPS_COSMOS_SDK_VERSION := $(shell cat go.sum | grep -E 'github.com/sacasnetwork/cosmos-sdk\s' | grep -v -e 'go.mod' | tail -n 1 | awk '{ print $$2; }')
...
$(BUILD_TARGETS): go.sum $(BUILDDIR)/
	CGO_ENABLED="1" go $@ $(BUILD_FLAGS) $(BUILD_ARGS) -buildvcs=false ./...
...
proto-download-deps:
	...
	git init && \
	git remote add origin "https://github.com/sacasnetwork/cosmos-sdk.git" && \
...
# Sacas repository and then clean up the contracts data.
...
# Compile the solidity contracts found in the Sacas repository.
contracts-compile:
