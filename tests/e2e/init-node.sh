...
# Check sacasd version to decide how to set the client configuration
# the older versions of sacasd accept less arguments
sdk_version=$(sacasd version --long | grep 'cosmos_sdk_version' | awk '{print $2}')
if [[ $sdk_version == *v0.4* ]]; then
	sacasd config chain-id "$CHAINID"
	sacasd config keyring-backend "$KEYRING"
else
	sacasd config set client chain-id "$CHAINID"
	sacasd config set client keyring-backend "$KEYRING"
fi

# Import keys from mnemonics
echo "$VAL_MNEMONIC" | sacasd keys add "$VAL_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO"

echo "$USER1_MNEMONIC" | sacasd keys add "$USER1_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO"
echo "$USER2_MNEMONIC" | sacasd keys add "$USER2_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO"
echo "$USER3_MNEMONIC" | sacasd keys add "$USER3_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO"
echo "$USER4_MNEMONIC" | sacasd keys add "$USER4_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO"
...
sacasd add-genesis-account "$(sacasd keys show "$VAL_KEY" -a --keyring-backend "$KEYRING")" 100000000000000000000000000asacas --keyring-backend "$KEYRING"
sacasd add-genesis-account "$(sacasd keys show "$USER1_KEY" -a --keyring-backend "$KEYRING")" 1000000000000000000000asacas --keyring-backend "$KEYRING"
sacasd add-genesis-account "$(sacasd keys show "$USER2_KEY" -a --keyring-backend "$KEYRING")" 1000000000000000000000asacas --keyring-backend "$KEYRING"
sacasd add-genesis-account "$(sacasd keys show "$USER3_KEY" -a --keyring-backend "$KEYRING")" 1000000000000000000000asacas --keyring-backend "$KEYRING"
sacasd add-genesis-account "$(sacasd keys show "$USER4_KEY" -a --keyring-backend "$KEYRING")" 1000000000000000000000asacas --keyring-backend "$KEYRING"
...
sacasd gentx "$VAL_KEY" 1000000000000000000000asacas --gas-prices ${BASEFEE}asacas --keyring-backend "$KEYRING" --chain-id "$CHAINID"
