...
import (
	...
	testnetwork "github.com/sacasnetwork/sacas/v20/testutil/integration/sacas/network"
...
func (m *Manager) BuildImage(name, version, dockerFile, contextDir string, args map[string]string) error {
	...
	opts := docker.BuildImageOptions{
		...
		NoCache: true,
		// name with tag, e.g. sacas:v9.0.0
	...
	err = m.pool.Retry(
		func() error {
			...
			if c.State.ExitCode != 0 {
				stdOut, stdErr, _ := m.GetLogs(resource.Container.ID)
				return fmt.Errorf(
					"can't start sacas node, container exit code: %d\n\n[error stream]:\n\n%s\n\n[output stream]:\n\n%s",
					c.State.ExitCode,
					stdErr,
					stdOut,
				)
...
// WaitForHeight queries the Sacas node every second until the node will reach the specified height.
...
// GetNodeHeight calls the Sacas CLI in the current node container to get the current block height
func (m *Manager) GetNodeHeight(ctx context.Context) (int, error) {
	cmd := []string{"sacasd", "q", "block"}
	...
	if !SacasVersions([]string{m.CurrentVersion, "v20.0.0"}).Less(0, 1) {
	...
	if errBuff.String() != "" {
		return 0, fmt.Errorf("sacas query error: %s", errBuff.String())
	}
...
// UnwrapBlockHeightPreV50 unwraps the block height from the output of the sacasd query block command
...
// UnwrapBlockHeightPostV50 unwraps the block height from the output of the sacasd query block command
...
// GetNodeVersion calls the Sacas CLI in the current node container to get the
// current node version
func (m *Manager) GetNodeVersion(ctx context.Context) (string, error) {
	exec, err := m.CreateExec([]string{"sacasd", "version"}, m.ContainerID())
	if err != nil {
	...
	if errBuff.String() != "" {
		return "", fmt.Errorf("sacas version error: %s", errBuff.String())
	}
...
func (m *Manager) getTimeoutCommit(ctx context.Context) (*big.Int, error) {
	exec, err := m.CreateExec([]string{"grep", `\s*timeout_commit =`, "/root/.sacasd/config/config.toml"}, m.ContainerID())
	if err != nil {
	...
	if errBuff.String() != "" {
		return common.Big0, fmt.Errorf("sacas version error: %s", errBuff.String())
	}
