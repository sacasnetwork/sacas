...
function checkTestEnv () {
  const argv = yargs(hideBin(process.argv))
    ...
    .example('$0 --network sacas', 'run all tests using sacas network')
    .example(
      '$0 --network sacas --allowTests=test1,test2',
      'run only test1 and test2 using sacas network'
    )
    .help('h')
    .alias('h', 'help')
    .describe('network', 'set which network to use: ganache|sacas')
    .describe(
      'batch',
    ...
    .boolean('verbose-log')
    .describe('verbose-log', 'print sacasd output, default false').argv
  ...
  if (!argv.network) {
    ...
    if (argv.network !== 'sacas' && argv.network !== 'ganache') {
      panic('network is invalid. Must be ganache or sacas')
    } else {
...
function loadTests (runConfig) {
  ...
  fs.readdirSync(path.join(__dirname, 'suites')).forEach((dirname) => {
    ...
    try {
      ...
      const needScripts = ['test-ganache', 'test-sacas']
...
function performTestSuite ({ testName, network }) {
  const cmd = network === 'ganache' ? 'test-ganache' : 'test-sacas'
...
function setupNetwork ({ runConfig, timeout }) {
  if (runConfig.network !== 'sacas') {
    // no need to start ganache. Truffle will start it
    return
  }

  // Spawn the sacas process

  const spawnPromise = new Promise((resolve, reject) => {
    const serverStartedLog = 'Starting JSON-RPC server'
    const serverStartedMsg = 'sacasd started'

    const sacasdProc = spawn('../e2e/init-node.sh', {
      cwd: __dirname,
      stdio: ['ignore', 'pipe', 'pipe']
    })

    logger.info(`Starting sacasd process... timeout: ${timeout}ms`)
    if (runConfig.verboseLog) {
      sacasdProc.stdout.pipe(process.stdout)
    }

    sacasdProc.stdout.on('data', (d) => {
  ...
  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error('Start sacasd timeout!')), timeout)
  })
