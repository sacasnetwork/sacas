...
def test_block(sacas_cluster, geth):
    # Perform block queries (eth_getBlockByNumber & eth_getBlockByHash)
    # on sacas and geth nodes, and compare the responses
    # Case 1: block without transactions
    get_blocks(sacas_cluster, geth, False)
    # Case 2: block with transactions
    get_blocks(sacas_cluster, geth, True)


def get_blocks(sacas_rpc_ws, geth, with_transactions):
    w3: Web3 = sacas_rpc_ws.w3
    eth_rpc = w3.provider
    ...
    sacas_blk = w3.eth.get_block(1)
    # Get existing block, no transactions
    eth_rsp = eth_rpc.make_request(
        "eth_getBlockByHash", [sacas_blk["hash"].hex(), with_transactions]
    )
...
def test_accounts(sacas_cluster, geth):
    w3: Web3 = sacas_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
    make_same_rpc_calls(eth_rpc, geth_rpc, "eth_accounts", [])


def test_syncing(sacas_cluster, geth):
    w3: Web3 = sacas_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
...
def test_coinbase(sacas_cluster, geth):
    w3: Web3 = sacas_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
...
def test_max_priority_fee(sacas_cluster, geth):
    w3: Web3 = sacas_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
...
def test_gas_price(sacas_cluster, geth):
    w3: Web3 = sacas_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
...
def test_block_number(sacas_cluster, geth):
    w3: Web3 = sacas_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
...
def test_balance(sacas_cluster, geth):
    w3: Web3 = sacas_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
    make_same_rpc_calls(
...
def test_get_storage_at(sacas_cluster, geth):
    w3: Web3 = sacas_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
    make_same_rpc_calls(
...
def test_get_proof(sacas_cluster, geth):
    # on sacas the proof query will fail for block numbers <= 2
...
def test_get_code(sacas_cluster, geth):
    w3: Web3 = sacas_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
    make_same_rpc_calls(
...
def test_get_block_transaction_count(sacas_cluster, geth):
    w3: Web3 = sacas_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
    make_same_rpc_calls(
...
def test_get_transaction(sacas_cluster, geth):
    w3: Web3 = sacas_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
    make_same_rpc_calls(
...
def test_get_transaction_receipt(sacas_cluster, geth):
    w3: Web3 = sacas_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
    make_same_rpc_calls(
...
def test_fee_history(sacas_cluster, geth):
    w3: Web3 = sacas_cluster.w3
    eth_rpc = w3.provider
    geth_rpc = geth.w3.provider
...
def test_estimate_gas(sacas_cluster, geth):
...
def test_incomplete_send_transaction(sacas_cluster, geth):
