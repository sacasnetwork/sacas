...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            [
                ADDRS["community"],
                [["transfer", "channel-0", [["sac", int(1e18)]], [], []]],
            ],
            [],
            "improper number of arguments",
            None,
        ...
        (
            ...
            None,
            [("transfer", "channel-0", [("sac", 1000000000000000000)], [], [])],
        ),
        (
            ...
            [
                (
                    "transfer",
                    "channel-0",
                    [("sac", 1000000000000000000), ("uatom", 1000000000000000000)],
                    [],
                    [],
                )
            ],
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            [
                ADDRS["signer2"],
                [["transfer", "channel-1", [["sac", int(1e18)]], [], []]],
            ],
            True,
            "channel not found",
            0,
        ...
        (
            ...
            [
                ADDRS["signer2"],
                [["transfer", "channel-0", [["sac", MAX_UINT256]], [], []]],
            ],
            False,
            "",
            MAX_UINT256,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            False,
            "",
            json.loads("""[{"denom": "sac", "amount": "2000000000000000000"}]"""),
        ),
        (
            ...
            "",
            json.loads(
                """[{"denom": "sac", "amount": "1000000000000000000"},"""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            False,
            "",
            json.loads("""[{"denom": "sac", "amount": "500000000000000000"}]"""),
        ),
        (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            None,
            int(1e18),
            json.loads(f"""[{{"denom": "sac", "amount": "{MAX_UINT256}"}}]"""),
        ),
        (
...
def test_ibc_transfer_with_authorization(
    ...
    dst_balance_prev = get_balance(
        ibc.chains["chainmain"], dst_address, SAC_IBC_DENOM
    )
    try:
    ...
    assert transfer_event.args.denom == "sac"
...
def test_ibc_transfer_from_eoa_through_contract(ibc):
    ...
    src_starting_balance = get_balance(ibc.chains["evmos"], src_adr, "sac")
    dest_starting_balance = get_balance(
        ibc.chains["chainmain"], dst_addr, SAC_IBC_DENOM
    )
    # Calling the actual transfer function on the custom contract
...
def test_ibc_transfer_from_eoa_with_internal_transfer(
    ...
    dst_addr = ibc.chains["chainmain"].cosmos_cli().address("signer2")
    # address that will escrow the sac tokens when transferring via IBC
    ...
    src_starting_balance = get_balance(ibc.chains["evmos"], src_bech32, "sac")
    dest_starting_balance = get_balance(
        ibc.chains["chainmain"], dst_addr, SAC_IBC_DENOM
    )
...
def test_ibc_multi_transfer_from_eoa_with_internal_transfer(
    ...
    # send some funds (100 sac) to the contract to perform
    # internal transfer within the tx
    src_bech32 = eth_to_bech32(src_addr)
    contract_bech32 = eth_to_bech32(eth_contract.address)

    # get starting balances to check after the tx
    src_starting_balance = get_balance(evmos, src_bech32, "sac")
    dest_starting_balance = get_balance(
        ibc.chains["chainmain"], dst_addr, SAC_IBC_DENOM
    )
...
def setup_interchain_sender_contract(
    ...
    fund_tx = evmos.cosmos_cli().transfer(
        src_bech32,
        contract_bech32,
        f"{initial_contract_balance}sac",
        gas_prices=f"{evmos_gas_price + 100000}sac",
        generate_only=True,
    )
