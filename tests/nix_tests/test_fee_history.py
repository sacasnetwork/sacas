...
def custom_sacas(tmp_path_factory):
    ...
    yield from setup_sacas(path, 26500)


@pytest.fixture(scope="module")
def custom_sacas_rocksdb(tmp_path_factory):
    path = tmp_path_factory.mktemp("fee-history-rocksdb")
    yield from setup_sacas_rocksdb(path, 26510)


@pytest.fixture(scope="module", params=["sacas", "sacas-rocksdb", "geth"])
def cluster(request, custom_sacas, custom_sacas_rocksdb, geth):
    """
    run on sacas, sacas built with rocksdb and geth
    """
