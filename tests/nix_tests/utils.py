...
OSMOSIS_POOLS = {
    "Sacas_Osmo": Path(__file__).parent / "osmosis/evmosOsmosisPool.json",
}
...
def eth_to_bech32(addr, prefix=SAC_ADDRESS_PREFIX):
...
def update_node_cmd(path, cmd, i):
    ...
    for section in ini.sections():
        if section == f"program:sac_1312-1-node{i}":
            ini[section].update(
                {
