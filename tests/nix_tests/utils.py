...
WSACAS_ADDRESS = Web3.toChecksumAddress("0xD4949664cD82660AaE99bEdc034a0deA8A0bd517")
TEST_CONTRACTS = {
    ...
    "InterchainSender": "sacas/testutil/contracts/InterchainSender.sol",
    "InterchainSenderCaller": "sacas/testutil/contracts/InterchainSenderCaller.sol",
    "ICS20I": "sacas/ics20/ICS20I.sol",
    "DistributionI": "sacas/distribution/DistributionI.sol",
    "DistributionCaller": "sacas/testutil/contracts/DistributionCaller.sol",
    "StakingI": "sacas/staking/StakingI.sol",
    "StakingCaller": "sacas/staking/testdata/StakingCaller.sol",
    "IStrideOutpost": "sacas/outposts/stride/IStrideOutpost.sol",
    "IOsmosisOutpost": "sacas/outposts/osmosis/IOsmosisOutpost.sol",
    "IERC20": "sacas/erc20/IERC20.sol",
}

OSMOSIS_POOLS = {
    "Sacas_Osmo": Path(__file__).parent / "osmosis/sacasOsmosisPool.json",
}
...
REGISTER_ERC20_PROP = {
    "messages": [
        {
            "@type": "/sacas.erc20.v1.MsgRegisterERC20",
...
def update_node_cmd(path, cmd, i):
    ...
    for section in ini.sections():
        if section == f"program:sacas_1317-1-node{i}":
            ini[section].update(
                {
...
def update_sacasd_and_setup_stride(modified_bin):
    def inner(path, base_port, config):  # pylint: disable=unused-argument
        update_sacas_bin(modified_bin)(path, base_port, config)
        setup_stride()(path, base_port, config)

    return inner


def update_sacas_bin(
    modified_bin, nodes=[0, 1]
):  # pylint: disable=dangerous-default-value
    """
    updates the sacas binary with a patched binary.
...
def debug_trace_tx(sacas, tx_hash: str):
    url = f"http://127.0.0.1:{ports.evmrpc_port(sacas.base_port(0))}"
    params = {
