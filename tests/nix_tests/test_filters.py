...
from .network import setup_custom_sacas, setup_sacas
from .utils import (
    ADDRS,
    CONTRACTS,
    deploy_contract,
...
def custom_sacas(tmp_path_factory):
    ...
    yield from setup_sacas(path, 26200)


@pytest.fixture(scope="module")
def sacas_indexer(tmp_path_factory):
...
@pytest.fixture(scope="module", params=["sacas", "geth", "sacas-ws", "enable-indexer"])
def cluster(request, custom_sacas, sacas_indexer, geth):
    """
    run on both sacas and geth
    """
