...
def custom_sacas(tmp_path_factory):
    ...
    cmd = [
        "nix-build",
        "--no-out-link",
        Path(__file__).parent / "configs/broken-sacasd.nix",
    ]
    print(*cmd)
    broken_binary = Path(subprocess.check_output(cmd).strip().decode()) / "bin/sacasd"
    print(broken_binary)
...
def test_rollback(custom_sacas):
    """
    ...
    print("stop node1")
    supervisorctl(custom_sacas.base_dir / "../tasks.ini", "stop", "sacas_1317-1-node1")

    print("do rollback on node1")
    cli1.rollback()

    print("switch to normal binary")
    update_node_cmd(custom_sacas.base_dir, "sacasd", 1)
    supervisorctl(custom_sacas.base_dir / "../tasks.ini", "update")
    wait_for_port(target_port)
