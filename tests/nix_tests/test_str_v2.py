...
# uatom from cosmoshub-2 -> cosmoshub-1 IBC representation on the Sacas chain.
...
# The ERC20 address of ATOM on Sacas
...
UPDATE_PARAMS_PROP = {
    "messages": [
        {
            "@type": "/sacas.erc20.v1.MsgUpdateParams",
            "authority": "sacas10d07y265gmmuvt4z0w9aw880jnsr700jcrztvm",
            "params": {
                "enable_erc20": True,
                "native_precompiles": [],
                "dynamic_precompiles": [],
...
def ibc(request, tmp_path_factory):
    ...
    network = prepare_network(path, name, [sacas_build, "cosmoshub-1", "cosmoshub-2"])
    yield from network
...
def test_str_v2_single_hop(ibc):
    ...
    old_dst_balance = get_balance(sacas, bech_dst, ATOM_IBC_DENOM)
    ...
    new_dest_balance = get_balance(sacas, bech_dst, ATOM_IBC_DENOM)
    erc_dest_balance = erc20_balance(w3, ATOM_1_ERC20_ADDRESS, sacas_addr)
...
def test_str_v2_multi_hop(ibc):
    ...
    sacas_old_balance = get_balance(sacas, bech_dst, ATOM_2_IBC_DENOM_MULTI_HOP)
    ...
    sacas_balance = get_balance(sacas, bech_dst, ATOM_2_IBC_DENOM_MULTI_HOP)
    dynamic_precompiles = sacas_cli.erc20_params()["params"]["dynamic_precompiles"]
    token_pairs = sacas_cli.get_token_pairs()

    # Here it's only one from the previous one we've registered in the first test
    assert sacas_old_balance + 50000 == sacas_balance
    assert len(dynamic_precompiles) == 1
    assert dynamic_precompiles[0] == ATOM_1_ERC20_ADDRESS
    assert len(token_pairs) == 2


def test_wsacas_precompile_transfer(ibc):
    """
    Test the ERC20 transfer from one signer to another using the now
    registered ERC20 precompiled contract for WSACAS.
    """
    assert_ready(ibc)
    ...
    w3 = sacas.w3
    sacas_balance = get_balance(sacas, bech_dst, src_denom)
    signer2_balance = erc20_balance(w3, WSACAS_ADDRESS, signer2)

    assert sacas_balance == signer2_balance

    receipt = erc20_transfer(
        w3, WSACAS_ADDRESS, signer1, signer2, 1000000, KEYS["signer1"]
    )
    assert receipt.status == 1

    signer_2_balance_after = erc20_balance(w3, WSACAS_ADDRESS, signer2)
    assert signer_2_balance_after == signer2_balance + 1000000

    sacas_balance_after = get_balance(sacas, bech_dst, src_denom)
    assert sacas_balance_after == sacas_balance + 1000000
...
def test_toggle_erc20_precompile(ibc):
    ...
    acc = sacas_cli.evm_account(contract_bech32_addr)
...
def update_erc20_params(sacas: Sacas, native_precomiles=[], dynamic_precompiles=[]):
