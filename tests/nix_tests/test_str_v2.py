...
def test_str_v2_single_hop(ibc):
    ...
    # should only exist the WSAC pair
...
def test_wevmos_precompile_transfer(ibc):
    ...
    registered ERC20 precompiled contract for WSAC.
    """
    assert_ready(ibc)
    ...
    signer2_balance = erc20_balance(w3, WSAC_ADDRESS, signer2)

    assert evmos_balance == signer2_balance

    receipt = erc20_transfer(
        w3, WSAC_ADDRESS, signer1, signer2, 1000000, KEYS["signer1"]
    )
    assert receipt.status == 1

    signer_2_balance_after = erc20_balance(w3, WSAC_ADDRESS, signer2)
...
def update_erc20_params(evmos: Sacas, native_precomiles=[], dynamic_precompiles=[]):
