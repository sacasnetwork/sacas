...
def create_snapshots_dir(
    ...
    for idx in range(n_nodes):
        data_snapshots_dir = path / "sac_1312-1" / f"node{idx}" / "data" / "snapshots"
        os.makedirs(data_snapshots_dir, exist_ok=True)


def setup_evmos_rocksdb(path, base_port, long_timeout_commit=False):
    """
    setup_evmos_rocksdb returns an Sacas chain compiled with RocksDB
...
def setup_custom_evmos(
    ...
    try:
        ...
        yield Sacas(
            path / "sac_1312-1", chain_binary=chain_binary or DEFAULT_CHAIN_BINARY
        )
    finally:
        os.killpg(os.getpgid(proc.pid), signal.SIGTERM)
        proc.wait()
...
def build_patched_evmosd(patch_nix_file):
    ...
    e.g. allow to register WSAC token
