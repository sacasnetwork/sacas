...
from .network import CosmosChain, Hermes, setup_custom_sacas
from .utils import ADDRS, eth_to_bech32, wait_for_port

# SACAS_IBC_DENOM IBC denom of asacas in crypto-org-chain
...
def prepare_network(tmp_path: Path, file: str, other_chains_names: List[str]):
    ...
    gen = setup_custom_sacas(tmp_path, 26700, Path(__file__).parent / file)
    sacas = next(gen)
    chains = {"sacas": sacas}
    # wait for grpc ready
    ...
    # relayer
    hermes = Hermes(sacas.base_dir.parent / "relayer.toml")

    chains_to_connect = ["sacas_1317-1"]

    # set up the other chains to connect to sacas
    for chain in other_chains_names:
        meta = IBC_CHAINS_META[chain]
        other_chain_name = meta["chain_name"]
        chain_instance = CosmosChain(
            sacas.base_dir.parent / other_chain_name, meta["bin"]
        )
        ...
        if "chainmain" not in other_chain_name:
            subprocess.run(
                [
                    ...
                    other_chain_name,
                    "--mnemonic-file",
                    sacas.base_dir.parent / "relayer.env",
                    "--overwrite",
                ],
                check=True,
            )
