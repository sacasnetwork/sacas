...
from .utils import (
    ...
    setup_stride,
    update_sacas_bin,
    update_sacasd_and_setup_stride,
    wait_for_fn,
    wait_for_port,
)

# asacas IBC representation on another chain connected via channel-0.
SACAS_IBC_DENOM = "ibc/8EAC8061F4499F03D2D1419A3E73D346289AE9DB89CAB1486B72539572B1915E"
# uosmo IBC representation on the Sacas chain.
...
# uatom from cosmoshub-1 IBC representation on the Sacas chain and on Cosmos Hub 2 chain.
...
EVM_CHAINS = ["sacas_1317", "chainmain-1"]
...
def get_sacas_generator(
    ...
    """
    setup sacas with custom config
    depending on the build
    """
    post_init_func = None
    if is_rocksdb:
        file = memiavl_config(tmp_path, file)
        gen = setup_custom_sacas(
            tmp_path,
            26710,
        ...
        if custom_scenario:
            ...
            modified_bin = build_patched_sacasd(custom_scenario)
            post_init_func = update_sacas_bin(modified_bin)
            if stride_included:
                post_init_func = update_sacasd_and_setup_stride(modified_bin)
            gen = setup_custom_sacas(
                tmp_path,
                26700,
                Path(__file__).parent / file,
...
def prepare_network(
    ...
    for chain in chain_names:
        ...
        # sacas is the first chain
        # set it up and the relayer
        if "sacas" in chain_name:
            # setup sacas with the custom config
            # depending on the build
            gen = get_sacas_generator(
                tmp_path,
                file,
                "-rocksdb" in chain,
            ...
            sacas = next(gen)  # pylint: disable=stop-iteration-return

            # setup relayer
            hermes = Hermes(tmp_path / "relayer.toml")

            # wait for grpc ready
            wait_for_port(ports.grpc_port(sacas.base_port(0)))  # sacas grpc
            chains["sacas"] = sacas
            continue
    ...
    sacas.supervisorctl("start", "relayer-demo")
...
def hermes_transfer(ibc, other_chain_name="chainmain-1", other_chain_denom="basecro"):
    assert_ready(ibc)
    # chainmain-1 -> sacas_1317-1
    my_ibc0 = other_chain_name
    my_ibc1 = "sacas_1317-1"
...
def setup_denom_trace(ibc):
    """
    Helper setup function to send some funds from chain-main to sacas
    to register the denom trace (if not registered already)
    """
    res = ibc.chains["sacas"].cosmos_cli().denom_traces()
    if len(res["denom_traces"]) == 0:
        amt = 100
        src_denom = "basecro"
        dst_addr = ibc.chains["sacas"].cosmos_cli().address("signer2")
