...
def test_evmos_ibc_transfer_native_denom(ibc, name, transfer_amt, err_contains):
    """
    test sending sac from evmos to crypto-org-chain using cli.
    """
    assert_ready(ibc)
    ...
    old_dst_balance = get_balance(chainmain, dst_addr, SAC_IBC_DENOM)
...
def test_evmos_ibc_transfer_ibc_denom(
    ...
    # submit proposal if limit was not set
    limits = cli.rate_limits()
    # expect to already have one rate limit (for 'sac') from the previous test
    if len(limits) == 1:
...
def add_rate_limit(evmos: Sacas, denom: str = "sac"):
    cli = evmos.cosmos_cli()
