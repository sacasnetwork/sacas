...
def custom_sacas(tmp_path_factory):
    ...
    yield from setup_sacas(path, 26800, long_timeout_commit=True)
...
def custom_sacas_rocksdb(tmp_path_factory):
    ...
    # run with long timeout commit to ensure all
    # txs are included in the same block
    yield from setup_sacas_rocksdb(path, 26810, long_timeout_commit=True)
...
def test_priority(sacas_cluster):
    """
...
def test_native_tx_priority(sacas_cluster):
    ...
    test_cases = [
        {
            ...
            "amount": "1000asacas",
            "gas_prices": f"{base_fee + PRIORITY_REDUCTION * 600000}asacas",
        ...
        },
        {
            ...
            "amount": "1000asacas",
            "gas_prices": f"{base_fee + PRIORITY_REDUCTION * 400000}asacas",
    ...
    for tc in test_cases:
        ...
        gas_price = int(tc["gas_prices"].removesuffix("asacas"))
