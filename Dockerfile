FROM golang:1.24.1-alpine3.20 AS build-env

ARG DB_BACKEND=goleveldb
ARG ROCKSDB_VERSION="9.3.1"

WORKDIR /go/src/github.com/sacas/sacas
...
COPY --from=build-env /go/src/github.com/sacas/sacas/build/sacasd /usr/bin/sacasd
...
RUN apk add --no-cache \
    ...
    lz4 \
    rclone \
    && addgroup -g 1000 sacas \
    && adduser -S -h /home/sacas -D sacas -u 1000 -G sacas
