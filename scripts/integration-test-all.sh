...
init_func() {
	...
	"$PWD"/build/evmosd add-genesis-account \
		"$("$PWD"/build/evmosd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000sac,1000000000000000000stake \
...
start_func() {
	...
	SAC_PID=$!
	echo "started evmos node, pid=$SAC_PID"
	# add PID to array
	arr+=("$SAC_PID")
...
stop_func() {
	SAC_PID=$i
	echo "shutting down node, pid=$SAC_PID ..."
