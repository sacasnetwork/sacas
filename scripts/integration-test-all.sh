...
## default port prefixes for sacasd
NODE_P2P_PORT="2660"
NODE_PORT="2663"
NODE_RPC_PORT="2666"
...
init_func() {
	"$PWD"/build/sacasd keys add $KEY"$i" --keyring-backend test --home "$DATA_DIR$i" --no-backup --algo "eth_secp256k1"
	"$PWD"/build/sacasd init $MONIKER --chain-id $CHAINID --home "$DATA_DIR$i"
	"$PWD"/build/sacasd add-genesis-account \
		"$("$PWD"/build/sacasd keys show "$KEY$i" --keyring-backend test -a --home "$DATA_DIR$i")" 1000000000000000000asacas,1000000000000000000stake \
		--keyring-backend test --home "$DATA_DIR$i"
	"$PWD"/build/sacasd gentx "$KEY$i" 1000000000000000000stake --chain-id $CHAINID --keyring-backend test --home "$DATA_DIR$i"
	"$PWD"/build/sacasd collect-gentxs --home "$DATA_DIR$i"
	"$PWD"/build/sacasd validate-genesis --home "$DATA_DIR$i"
...
start_func() {
	...
	"$PWD"/build/sacasd start --pruning=nothing --rpc.unsafe \
	...
	SACAS_PID=$!
	echo "started sacas node, pid=$SACAS_PID"
	# add PID to array
	arr+=("$SACAS_PID")
...
if [[ -z $TEST || $TEST == "rpc" || $TEST == "pending" ]]; then
	...
	for i in $(seq 1 "$TEST_QTD"); do
		...
		echo "going to test sacas node $HOST_RPC ..."
...
stop_func() {
	SACAS_PID=$i
	echo "shutting down node, pid=$SACAS_PID ..."
