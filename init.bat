...
rem Change parameter token denominations to sac
cat %GENESIS% | jq ".app_state[\"staking\"][\"params\"][\"bond_denom\"]=\"sac\""   >   %TMPGENESIS% && move %TMPGENESIS% %GENESIS%
cat %GENESIS% | jq ".app_state[\"crisis\"][\"constant_fee\"][\"denom\"]=\"sac\"" > %TMPGENESIS% && move %TMPGENESIS% %GENESIS%
cat %GENESIS% | jq ".app_state[\"gov\"][\"deposit_params\"][\"min_deposit\"][0][\"denom\"]=\"sac\"" > %TMPGENESIS% && move %TMPGENESIS% %GENESIS%
cat %GENESIS% | jq ".app_state[\"mint\"][\"params\"][\"mint_denom\"]=\"sac\"" > %TMPGENESIS% && move %TMPGENESIS% %GENESIS%
...
sacasd add-genesis-account %KEY% 100000000000000000000000000sac --keyring-backend %KEYRING%

rem Sign genesis transaction
sacasd gentx %KEY% 1000000000000000000000sac --keyring-backend %KEYRING% --chain-id %CHAINID%
...
sacasd start --pruning=nothing %TRACE% --log_level %LOGLEVEL% --minimum-gas-prices=0.00001sac
