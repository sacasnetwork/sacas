...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	# Set client config
	sacasd config keyring-backend "$KEYRING" --home "$HOMEDIR"
	sacasd config chain-id "$CHAINID" --home "$HOMEDIR"

	# myKey address 0x7cb61d4117ae31a12e393a1cfa3bac666481d02e | sacas10jmp6sgh4cc6zt3e8gw05wavvejgr5pwjnpcky
	VAL_KEY="mykey"
	VAL_MNEMONIC="gesture inject test cycle original hollow east ridge hen combine junk child bacon zero hope comfort vacuum milk pitch cage oppose unhappy lunar seat"

	# dev0 address 0xc6fe5d33615a1c52c08018c47e8bc53646a0e101 | sacas1cml96vmptgw99syqrrz8az79xer2pcgp84pdun
	USER1_KEY="dev0"
	USER1_MNEMONIC="copper push brief egg scan entry inform record adjust fossil boss egg comic alien upon aspect dry avoid interest fury window hint race symptom"

	# dev1 address 0x963ebdf2e1f8db8707d05fc75bfeffba1b5bac17 | sacas1jcltmuhplrdcwp7stlr4hlhlhgd4htqh3a79sq
	USER2_KEY="dev1"
	USER2_MNEMONIC="maximum display century economy unlock van census kite error heart snow filter midnight usage egg venture cash kick motor survey drastic edge muffin visual"

	# dev2 address 0x40a0cb1C63e026A81B55EE1308586E21eec1eFa9 | sacas1gzsvk8rruqn2sx64acfsskrwy8hvrmafqkaze8
	USER3_KEY="dev2"
	...
	# dev3 address 0x498B5AeC5D439b733dC2F58AB489783A23FB26dA | sacas1fx944mzagwdhx0wz7k9tfztc8g3lkfk6rrgv6l
	USER4_KEY="dev3"
	USER4_MNEMONIC="doll midnight silk carpet brush boring pluck office gown inquiry duck chief aim exit gain never tennis crime fragile ship cloud surface exotic patch"

	# Import keys from mnemonics
	echo "$VAL_MNEMONIC" | sacasd keys add "$VAL_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"

	# Store the validator address in a variable to use it later
	node_address=$(sacasd keys show -a "$VAL_KEY" --keyring-backend "$KEYRING" --home "$HOMEDIR")

	echo "$USER1_MNEMONIC" | sacasd keys add "$USER1_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER2_MNEMONIC" | sacasd keys add "$USER2_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER3_MNEMONIC" | sacasd keys add "$USER3_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"
	echo "$USER4_MNEMONIC" | sacasd keys add "$USER4_KEY" --recover --keyring-backend "$KEYRING" --algo "$KEYALGO" --home "$HOMEDIR"

	# Set moniker and chain-id for Sacas (Moniker can be anything, chain-id must be an integer)
	sacasd init $MONIKER -o --chain-id "$CHAINID" --home "$HOMEDIR"
	...
	sacasd add-genesis-account "$(sacasd keys show "$VAL_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 100000000000000000000000000asacas --keyring-backend "$KEYRING" --home "$HOMEDIR"
	sacasd add-genesis-account "$(sacasd keys show "$USER1_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000asacas --keyring-backend "$KEYRING" --home "$HOMEDIR"
	sacasd add-genesis-account "$(sacasd keys show "$USER2_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000asacas --keyring-backend "$KEYRING" --home "$HOMEDIR"
	sacasd add-genesis-account "$(sacasd keys show "$USER3_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000asacas --keyring-backend "$KEYRING" --home "$HOMEDIR"
	sacasd add-genesis-account "$(sacasd keys show "$USER4_KEY" -a --keyring-backend "$KEYRING" --home "$HOMEDIR")" 1000000000000000000000asacas --keyring-backend "$KEYRING" --home "$HOMEDIR"
	...
	sacasd gentx "$VAL_KEY" 1000000000000000000000asacas --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$HOMEDIR"
	...
	sacasd validate-genesis --home "$HOMEDIR"
