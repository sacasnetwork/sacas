...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	# Set client config
	sacasd config set client chain-id "$CHAINID" --home "$HOMEDIR"
	sacasd config set client keyring-backend "$KEYRING" --home "$HOMEDIR"
	...
	sacasd gentx "$VAL_KEY" 1000000000000000000000asacas --gas-prices ${BASEFEE}asacas --keyring-backend "$KEYRING" --chain-id "$CHAINID" --home "$HOMEDIR"
