...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	for KEY in "${KEYS[@]}"; do
		sacasd keys add "$KEY" --keyring-backend $KEYRING --algo $KEYALGO --home "$HOMEDIR"
	done
	...
	claims_key=${KEYS[0]}
	node_address=$(sacasd keys show "$claims_key" --keyring-backend $KEYRING --home "$HOMEDIR" | grep "address" | cut -c12-)
	...
	for KEY in "${KEYS[@]}"; do
		sacasd add-genesis-account "$KEY" 100000000000000000000000000asacas --keyring-backend $KEYRING --home "$HOMEDIR"
	done
	...
	sacasd gentx "${KEYS[0]}" 1000000000000000000000asacas --keyring-backend $KEYRING --chain-id $CHAINID --home "$HOMEDIR"
...
sacasd start --metrics "$TRACE" --log_level $LOGLEVEL --minimum-gas-prices=0.0001asacas --json-rpc.api eth,txpool,personal,net,debug,web3 --api.enable --json-rpc.enable true --home "$HOMEDIR"
