...
- Use the Sacas security email ([security@sacas.org](mailto:security@sacas.org)) to disclose all vulnerabilities,
...
Sacas uses the following disclosure process:
...
2. The team determines the vulnerability’s potential impact on Sacas.
...
- For `High` and `Critical`:
    1. Patches are prepared for supported releases of Sacas in a
    ...
    4. The nodes and validators update their Sacas and Ethermint dependencies to use these releases.
    5. A week (or less) after the security vulnerability has been patched on Sacas,
...
including but not limited to Sacas,
as secure as possible.

### Payment Process

The payment process will be executed according to Sacas SAFU for `Critical` and `High` severity vulnerabilities.
Payouts can only be executed in accordance and under supervision of the Sacas Operations team and only once the
...
KYC/KYB result
(Accepted or Rejected) to the Sacas Core Team. The Sacas Core team does not have access to any of the information
...
The team commits to releasing security patch releases for the latest release that Sacas is running.
...
are welcomed to backport security patches to older versions for their own use, the Sacas team reserves
...
The Sacas Security Team is constantly being monitored.
If you need to reach out to the team directly,
please reach out via email: [security@sacas.org](mailto:security@sacas.org)
